...
let calcEquation = (equations, values, queries) => {
  ...
  const bfs = (dividend: string, divisor: string): number => {
    ...
    const queue: QueueItem[] = [{ curDividend: dividend, product: 1 }];
    while (queue.length) {
      const { curDividend, product } = queue.shift() as QueueItem;
      if (curDividend === divisor) return product;
      const curDivisors = divMap.get(curDividend) || [];
      for (const [curDivisor, weight] of curDivisors) {
        if (visited.has(curDivisor)) continue;
        visited.add(curDivisor);
        queue.push({ curDividend: curDivisor, product: product * weight });
      }
    }
    return -1;
  };
